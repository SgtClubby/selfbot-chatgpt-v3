...
/*
  ...
  msg.reply(`Oh, I really love ${response.collectedMsg} too!`);

  If you want to delete the original command message, do: 
  const response = await awaitReply(msg, "Favourite Food?");
  msg.reply(`Oh, I really love ${response.content} too!`);
  response.sentMsg.delete();

*/
async function awaitReply(msg, q, del, limit = 60000) {
  ...
  try {
    ...
    return { response, questionMessage, message };
  } catch (e) {
    return false;
  }
}
